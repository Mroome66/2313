...
GND = 50            #уровень земли в окне с игрой
...
class Ball:
    ...
    def move(self):
        ...
        if self.y - self.vy >= (HEIGHT - GND) - self.r:
            self.y = (HEIGHT - GND) - self.r - 0.1       # слагаемое -0.1 для компенсации флуктуаций при остановке
            self.vy *= -0.5
            self.vx *= 0.5
        else:
            self.vy -= 1
...
class Gun:
    ...
    def power_up(self):
        '''
        Увеличивает мощность выстрела при зажатии игроком ЛКМ.
        '''
        if self.f2_on:
